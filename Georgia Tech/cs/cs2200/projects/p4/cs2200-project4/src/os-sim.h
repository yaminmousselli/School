 ...
 * These are integer values but it's helpful to give meaning to these integer values
 * by giving them names so you know what the values correspond to.
 */
typedef enum {
    PROCESS_NEW = 0,
    PROCESS_READY,
...
/*
 * force_preempt() preempts a running process before its time_slice expires.
